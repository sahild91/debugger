...
export class FlashCommand {
  ...
  async execute(options: FlashOptions = {}): Promise<FlashResult> {
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `Target board: ${targetBoard.friendlyName}`
      );
  ...
  private async validatePrerequisites(): Promise<void> {
    ...
    if (boards.length === 0) {
      throw new Error(
        "No boards detected. Please connect a board and try again."
      );
    }
  }

  private async getTargetBoard(
    ...
    if (preferredPort) {
      ...
      if (board) {
        ...
        this.outputChannel.appendLine(
          `Warning: Preferred port ${preferredPort} not found`
        );
      }
    }
    ...
    if (connectedPorts.length > 0) {
      const connectedBoard = boards.find((b) =>
        connectedPorts.includes(b.path)
      );
      if (connectedBoard) {
  ...
  private async executeFlash(config: {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.outputChannel.appendLine(
        `Flashing ${config.binaryPath} to ${config.board.path}`
      );
  ...
  private showFlashResults(result: FlashResult): void {
    ...
    if (success) {
      ...
      vscode.window.showInformationMessage(
        `Flash completed successfully in ${(flashTime / 1000).toFixed(
          1
        )}s${sizeInfo}`
      );
    } else {
  ...
  private async convertBinaryFormat(
    inputPath: string,
    outputFormat: "bin" | "hex" | "elf"
  ): Promise<string> {
    ...
    this.outputChannel.appendLine(
      `Binary conversion: ${inputPath} -> ${outputFormat} (placeholder)`
    );
  ...
  private validateBinaryFile(binaryPath: string): {
    valid: boolean;
    format?: string;
    size?: number;
  } {
    try {
      ...
      if (stats.size > maxSize) {
        this.outputChannel.appendLine(
          `Warning: Binary file is unusually large (${stats.size} bytes)`
        );
      }
